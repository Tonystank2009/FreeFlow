...
struct ContentView: View {
    ...
    private let accessibilityRestartFlagKey = "FreeFlow_AccessibilityRestartPending"
    private let hasAutoRestartedForAccessibilityKey = "FreeFlow_HasAutoRestartedForAccessibility"
    ...
    private func applyShortcutStateChanges<Content: View>(to view: Content) -> some View {
        view
            .onAppear {
                ...
                LicenseManager.shared.bootstrap()
            }
            .sheet(isPresented: self.$licenseManager.isUnlockPromptPresented) {
                UnlockView()
            }
    ...
    private var sidebarContent: some View {
        ...
        .clipped()
        .navigationTitle(self.settingsNavigation.isPresented ? "Settings" : "FreeFlow")
    ...
    private var settingsSidebarView: some View {
        VStack(spacing: 0) {
            ...
            .help("Back to FreeFlow")
            .accessibilityLabel("Back to FreeFlow")
    ...
    private var helpEntryButton: some View {
        ...
        .help("Open FreeFlow Help")
        .accessibilityLabel("Help")
        .accessibilityHint("Opens FreeFlow documentation in your default browser")
    }
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                if self.asr.micStatus == .notDetermined {
                    ...
                    self.instructionStep(number: "2", text: "Find **FreeFlow** in the microphone list")
                    self.instructionStep(number: "3", text: "Toggle **FreeFlow ON** to allow access")
                }
            }
            .padding(.leading, 4)
        }
    ...
    private func stopAndProcessTranscription(route: DictationOutputRoute = .normal) async {
        ...
        // When FreeFlow itself is frontmost, the bound editor already receives `finalText`.
...
extension ContentView {
    ...
    private func positionWindowBesideSystemSettings(requestID: UUID) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.45) {
            ...
            guard let window = NSApp.windows.first(where: { $0.isVisible && $0.title == "FreeFlow" }) ?? NSApp.keyWindow else {
                return
            }
    ...
    private func cancelAccessibilityPermissionFlow() {
        ...
        (NSApp.windows.first { $0.isVisible && $0.title == "FreeFlow" } ?? NSApp.keyWindow)?
    ...
    private var draggableAccessibilityAppURL: URL {
        ...
        let installedURL = URL(fileURLWithPath: "/Applications/FreeFlow.app")

...
private struct MicrophoneChangeOverlayView: View {
    ...
    private var explanation: String {
        ...
        if self.notice.presentation == .startupSelection {
            return "FreeFlow will try this microphone first when you dictate."
        }
        if self.notice.presentation == .selectionChange {
            return "FreeFlow selected this microphone for capture."
        }
        return "FreeFlow confirmed this microphone is capturing audio."
    }
}
...
private struct AutomaticDictionaryCorrectionOverlayView: View {
    ...
    private var choiceContent: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text("Save only this correction, or teach FreeFlow other pronunciations.")
                .font(.system(size: 11))
                .foregroundStyle(.white.opacity(0.58))
                .lineLimit(1)
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 9) {
                ...
                if self.session.isReady {
                    Label("FreeFlow got it right 3 times in a row.", systemImage: "checkmark.circle.fill")
                    ...
                    VStack(alignment: .leading, spacing: 5) {
                        ...
                        self.trainingInstruction(
                            number: 2,
                            text: "Say the word, then pause. FreeFlow captures it and listens again."
                        )

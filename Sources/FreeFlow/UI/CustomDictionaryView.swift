...
struct CustomDictionaryView: View {
            ...
            VStack(alignment: .leading, spacing: self.theme.metrics.spacing.lg) {
                HStack(alignment: .center, spacing: self.theme.metrics.spacing.md) {
                    ...
                    VStack(alignment: .leading, spacing: 3) {
                        ...
                        Text("Show FreeFlow the right spelling, by voice or by typing.")
    ...
    private var trainReplacementComposer: some View {
        VStack(alignment: .leading, spacing: self.theme.metrics.spacing.sm) {
            TextField("Type the correct text, e.g. FreeFlow", text: self.$trainingReplacement)
    ...
    private var manualReplacementField: some View {
        VStack(alignment: .leading, spacing: self.theme.metrics.spacing.sm) {
            Text("Change it to")
                .font(self.theme.typography.captionStrong)
            TextField("FreeFlow", text: self.$manualReplacement)
                .dictionaryInputChrome()
                .onSubmit { self.addManualReplacementIfValid() }
    ...
    private var trainingRecorderPanel: some View {
        VStack(alignment: .leading, spacing: self.theme.metrics.spacing.md) {
            Text("Teach FreeFlow your pronunciation")
            ...
            if self.trainingAlreadyCorrectWithoutReplacement {
                ...
                Label(
                    self.activePronunciationMatching
                        ...
                        : "FreeFlow recognized \(self.trainingTargetReference) 3 times in a row.",
                ...
                VStack(alignment: .leading, spacing: 7) {
                    ...
                    self.trainingInstruction(
                        number: 3,
                        text: "Say \(self.trainingTargetReference) naturally, then pause. FreeFlow records and listens again automatically."
                    )
                    self.trainingInstruction(
                        number: 4,
                        text: self.activePronunciationMatching
                            ? "Repeat 3 times to teach FreeFlow how your voice sounds."
        ...
        ThemedCard(style: .standard, hoverEffect: false) {
            HStack(alignment: .center, spacing: self.theme.metrics.spacing.md) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    ...
                    Text("Words and phrases FreeFlow will correct automatically.")
                        .font(self.theme.typography.caption)
                        .foregroundStyle(self.theme.palette.secondaryText)
                }
                .frame(maxWidth: .infinity, alignment: .leading)
        ...
        VStack(alignment: .leading, spacing: self.theme.metrics.spacing.lg) {
            HStack(alignment: .top, spacing: self.theme.metrics.spacing.md) {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text("FreeFlow automatically corrects these words and phrases.")
                        .font(self.theme.typography.caption)
                        .foregroundStyle(self.theme.palette.secondaryText)
                }
    ...
    private var boostWordEditor: some View {
        VStack(alignment: .leading, spacing: self.theme.metrics.spacing.md) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                Text("Word or Phrase")
                    .font(self.theme.typography.captionStrong)
                TextField("FreeFlow", text: self.$boostTermText)
                    .font(self.theme.typography.bodySmall)
                    .dictionaryInputChrome()
                ...
                VStack(alignment: .leading, spacing: self.theme.metrics.spacing.lg) {
                    ...
                    HStack(alignment: .top, spacing: self.theme.metrics.spacing.md) {
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            Text("Try Saying")
                                .font(self.theme.typography.captionStrong)
                            Text("Examples of what FreeFlow will type.")
                                .font(self.theme.typography.caption)
                                .foregroundStyle(self.theme.palette.secondaryText)
        ...
        VStack(alignment: .leading, spacing: self.theme.metrics.spacing.sm) {
            ...
            if self.punctuationRules.isEmpty {
                self.dictionaryEmptyState(
                    ...
                    detail: "Add what you say and what FreeFlow should type."
                )
            } else {
            ...
            if self.isEnabled {
                HStack(alignment: .top, spacing: 8) {
                    Image(systemName: "flask.fill")
                        .foregroundStyle(self.theme.palette.accent)
                    Text("Research Preview: Compares how your voice sounds instead of only the words FreeFlow hears. Results may vary.")
                        .font(self.theme.typography.caption)
                        .foregroundStyle(self.theme.palette.secondaryText)
                        .fixedSize(horizontal: false, vertical: true)
                }
    ...
    static func readinessCaption(
        ...
        if isReady {
            return usesVoiceMatching
                ? "Ready. FreeFlow learned how \(target) sounds in your voice."
                : "Ready. FreeFlow got \(target) right 3 times in a row."
        }
        return usesVoiceMatching
            ? "Say \(target) 3 times to unlock Add Replacement."
            : "Keep trying until FreeFlow gets \(target) right 3 times in a row."
    }
}
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                Text("This is what will appear in the final transcription.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                TextField("FreeFlow", text: self.$replacement)
                    .dictionaryInputChrome()

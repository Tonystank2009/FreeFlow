...
struct FeedbackView: View {
    ...
    var body: some View {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        HStack(spacing: 12) {
                            ...
                            VStack(alignment: .leading, spacing: 4) {
                                ...
                                Text("Your feedback helps us make FreeFlow even better")
                                    .font(.system(size: 14))
                        ...
                        HStack(spacing: 12) {
                            ...
                            HStack(spacing: 10) {
                                ...
                                if let sponsorURL = URL(string: "https://github.com/sponsors/adhyanshupadhyaya") {
                                    Link(destination: sponsorURL) {
                                        HStack(spacing: 8) {
                                            Image(systemName: "heart.fill")
                                            Text("Support FreeFlow")
                                                .fontWeight(.semibold)
                                        }
                                        .font(.system(size: 14))
                                        .padding(.horizontal, 20)
        ...
        .alert("Feedback Sent", isPresented: self.$showFeedbackConfirmation) {
            ...
            Text("Thank you for helping us improve FreeFlow.")
        }
    ...
    private func submitFeedback(data: [String: Any]) async -> Bool {
        // No feedback service by default — upstream's pointed at their own
        // backend. Without one configured, send the user to GitHub issues
        // rather than quietly dropping (or misdelivering) what they wrote.
        guard let endpoint = Brand.feedbackEndpoint, let url = URL(string: endpoint) else {
            DebugLogger.shared.info(
                "No feedback endpoint configured; opening GitHub issues instead",
                source: "FeedbackView"
            )
            await MainActor.run { NSWorkspace.shared.open(Brand.issuesURL) }
            return false
        }

...
extension AIEnhancementSettingsView {
    ...
    private func privateAIPrefixCacheRow(isBusy: Bool) -> some View {
        HStack(alignment: .center, spacing: 8) {
            ...
            Toggle("", isOn: self.privateAIPrefixCacheBinding)
                ...
                .labelsHidden()
                .disabled(isBusy)
                .help("Keeps On-device AI ready so your first dictation finishes sooner.")
    ...
    private func privateAIBackendPicker(isBusy: Bool) -> some View {
        ...
        .labelsHidden()
        .disabled(isBusy)
        .help("Local On-device AI runtime. Default is MLX on Apple Silicon.")
    }
    ...
    private func privateAIBoostRow(isBusy: Bool) -> some View {
        HStack(alignment: .center, spacing: 8) {
            ...
            Toggle("", isOn: self.privateAIBoostBinding)
                ...
                .labelsHidden()
                .disabled(isBusy)
                .help("Uses extra local acceleration so On-device AI finishes faster.")
    ...
    private var privateAIBoostBinding: Binding<Bool> {
        Binding(
            ...
            set: { enabled in
                ...
                Task { @MainActor in
                    await PrivateAIIntegrationService.shared.unloadCachedRuntime(
                        reason: enabled ? "On-device AI Boost enabled" : "On-device AI Boost disabled"
                    )
                    self.viewModel.refreshProviderItems()
                }
            }
    ...
    private var privateAIContextTokenLimitBinding: Binding<Int> {
        Binding(
            ...
            set: { value in
                ...
                Task { @MainActor in
                    await PrivateAIIntegrationService.shared.unloadCachedRuntime(
                        reason: "On-device AI context changed"
                    )
                    self.viewModel.refreshProviderItems()
                }
            }
    ...
    private func resetPrivateAIVerification(for model: PrivateAIRegisteredModel) {
        ...
        self.privateAILoadState = .idle
        Task { @MainActor in
            await PrivateAIIntegrationService.shared.unloadCachedRuntime(reason: "On-device AI verification reset")
            if PrivateAIIntegrationService.isModelInstalled(model) {
                self.privateAILoadState = .idle
            }
            self.viewModel.refreshProviderItems()
    ...
    private func setPrivateAIBackendPreference(_ preference: SettingsStore.PrivateAIBackendPreference) {
        ...
        Task { @MainActor in
            await PrivateAIIntegrationService.shared.unloadCachedRuntime(
                reason: "On-device AI backend changed to \(preference.displayName)"
            )
    ...
    private func privateAIContextControl(isBusy: Bool) -> some View {
        ...
        .fixedSize()
        .help("How much raw dictation On-device AI can clean at once. Higher values help long transcripts but use more RAM and can slow first response.")
    }

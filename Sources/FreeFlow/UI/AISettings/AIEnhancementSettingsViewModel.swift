...
final class AIEnhancementSettingsViewModel: ObservableObject {
    ...
    private func probeKeychainAccess() -> KeychainAccessCheckResult {
        let service = "com.freeflowapp.provider-api-keys"
    ...
    private func keychainPermissionExplanation(for status: OSStatus) -> String {
        var message = "FreeFlow stores provider API keys securely in your macOS Keychain but does not currently have permission to access it."
        if let detail = SecCopyErrorMessageString(status, nil) as String? {
            message += "\n\nmacOS reported: \(detail) (\(status))"
        }
        message += "\n\nClick \"Always Allow\" when the Keychain prompt appears, or open Keychain Access > login > Passwords, locate the FreeFlow entry, and grant access."
        return message
    }

    private func keychainPersistenceExplanation(for error: Error) -> String {
        var message = "FreeFlow could not save the API key to your macOS Keychain, so this provider was not verified."
    ...
    func presentKeychainAccessAlert(message: String) {
        let msg = message.isEmpty
            ? "FreeFlow stores provider API keys securely in your macOS Keychain. Please grant access by choosing \"Always Allow\" when prompted."
            : message

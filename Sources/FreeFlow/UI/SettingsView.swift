...
struct SettingsView: View {
                    ...
                    VStack(alignment: .leading, spacing: 14) {
                        ...
                        VStack(spacing: 16) {
                            ...
                            self.settingsToggleRow(
                                ...
                                description: "Automatically start FreeFlow when you log in",
                            ...
                            self.settingsToggleRow(
                                ...
                                description: "When off, FreeFlow starts silently in the menu bar at login. Opening the app yourself always shows the window.",
                                isOn: Binding(
                            ...
                            self.settingsToggleRow(
                                ...
                                description: "Keep FreeFlow in the menu bar only (hides Dock icon and Cmd+Tab entry)",
                            ...
                            HStack(spacing: 10) {
                                        ...
                                        do {
                                            ...
                                            if let pmkError = error as? PMKError, pmkError.isCancelled {
                                                ...
                                                msg.informativeText = isBeta
                                                    ...
                                                    : "You're already running the latest version of FreeFlow."
                                            } else {
                                ...
                                Button(self.rollbackVersion.isEmpty ? "Rollback" : "Rollback to \(self.rollbackVersion)") {
                                    ...
                                    confirm.informativeText = "This will restore a previous app version and relaunch FreeFlow."
                                    ...
                                    Task {
                                        ...
                                        do {
                                            ...
                                            await MainActor.run {
                                                ...
                                                success.informativeText = "Rolled back to \(targetVersion). FreeFlow will relaunch shortly."
                            ...
                            VStack(alignment: .leading, spacing: 12) {
                                ...
                                VStack(spacing: 12) {
                                    ...
                                    self.optionToggleRow(
                                        ...
                                        description: "Automatically pause currently playing audio/video when transcription starts. Resumes only if FreeFlow paused it.",
                                        isOn: Binding(
                                    ...
                                    self.optionToggleRow(
                                        ...
                                        description: "Share anonymous daily feature, onboarding, and model metrics. " +
                                            "When off, FreeFlow records one anonymous activity signal per day and sends them weekly to measure active use. " +
                    ...
                    VStack(alignment: .leading, spacing: 14) {
                        ...
                        VStack(alignment: .leading, spacing: 12) {
                            ...
                            self.optionToggleRow(
                                ...
                                description: "Show an alert when FreeFlow changes or loses its microphone.",
                                isOn: Binding(
                    ...
                    VStack(alignment: .leading, spacing: 14) {
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            Text("The debug log contains detailed information about app operations and can help with troubleshooting.")
                                .font(self.theme.typography.bodySmall)
                                .foregroundStyle(self.settingsSecondaryText)
                            Text("Crash diagnostics are written to Library/Logs/FreeFlow/FreeFlow.log by default.")
                                .font(self.theme.typography.bodySmall)
                                .foregroundStyle(self.settingsSecondaryText)
                        }
                    }
    ...
    private func performBackupExport() async {
        do {
            ...
            self.presentInfoAlert(
                ...
                message: "Saved your FreeFlow backup to:\n\(url.path)"
            )
        } catch {
    ...
    private func applyAudioHistoryBudget() {
        ...
        if self.audioHistoryUsageBytes > newBudgetBytes {
            ...
            confirm.informativeText = """
            This budget is below current audio usage. FreeFlow will delete the oldest saved audio first and keep transcript history.
            """
            confirm.alertStyle = .warning
...
private extension SettingsView {
    ...
    var microphonePrioritySection: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("FreeFlow tries microphones from top to bottom. Drag to reorder; unavailable devices keep their place.")
                .font(self.theme.typography.bodySmall)
                .foregroundStyle(self.settingsSecondaryText)
                .fixedSize(horizontal: false, vertical: true)
        }
    ...
    var microphoneQualityGuidance: some View {
        if self.selectedInputDevice?.isBluetooth == true {
            ...
            self.microphoneQualityGuidanceRow(
                message: "This order applies only to FreeFlow and does not change your macOS input.",
...
struct AnalyticsConfirmationView: View {
    ...
    private var contactInfoText: AttributedString {
        ...
        if let emailRange = text.range(of: "GitHub") {
            text[emailRange].link = Brand.issuesURL

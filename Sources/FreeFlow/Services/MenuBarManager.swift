...
final class MenuBarManager: NSObject, ObservableObject, NSMenuDelegate {
    ...
    private func buildMenuStructure() {
        ...
        // Open Main Window
        let openItem = NSMenuItem(title: "Open FreeFlow", action: #selector(openMainWindow), keyEquivalent: "")
    ...
    @objc private func rollbackToPreviousVersion(_ sender: Any?) {
        ...
        confirm.informativeText = "This will restore the backup and relaunch FreeFlow."
        ...
        Task { @MainActor in
            do {
                ...
                success.informativeText = "Rolled back to \(availableVersion). FreeFlow will relaunch shortly."
    ...
    private func isFluidMainWindow(_ window: NSWindow) -> Bool {
        ...
        return window.title == "FreeFlow" || window.title.contains("FreeFlow")
    }

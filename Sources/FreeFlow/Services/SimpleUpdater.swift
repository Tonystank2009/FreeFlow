...
final class SimpleUpdater {
    ...
    private func showUpdateInstallStatus(version: String) {
        ...
        panel.title = "Installing FreeFlow \(version)"
        ...
        let title = NSTextField(labelWithString: "Installing FreeFlow \(version)")
        title.frame = NSRect(x: 92, y: 76, width: 304, height: 24)
        title.font = .systemFont(ofSize: 16, weight: .semibold)
        content.addSubview(title)

        let detail = NSTextField(wrappingLabelWithString: "Downloading the update. FreeFlow will restart automatically.")

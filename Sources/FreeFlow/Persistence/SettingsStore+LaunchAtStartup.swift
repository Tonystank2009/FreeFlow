...
extension SettingsStore {
        ...
        do {
            ...
            if self.launchAtStartupEnabled != enabled {
                let mismatchMessage = enabled
                    ? "macOS did not enable FreeFlow in Login Items. Check System Settings > General > Login Items."
                    : "macOS still shows FreeFlow in Login Items. Check System Settings > General > Login Items."
    ...
    private func launchAtStartupFailureMessage(for error: Error, enabling: Bool) -> String {
        ...
        {
            return "FreeFlow could not \(action) launch at startup. This build may not be signed correctly for macOS Login Items."
        }

        if lowercasedDescription.contains("approval") ||
            lowercasedDescription.contains("authorize")
        {
            return "macOS needs approval before FreeFlow can \(action) launch at startup. Check System Settings > General > Login Items."
        }

        return "FreeFlow could not \(action) launch at startup. macOS reported: \(nsError.localizedDescription)"
    }
    ...
    private func runLaunchAtStartupAppleScript(_ source: String) throws {
        ...
        guard let script = NSAppleScript(source: source) else {
            throw NSError(
                domain: "FreeFlowLaunchAtStartup",
                code: 1,
...
private enum LaunchAtStartupSystemState {
    ...
    var message: String {
        switch self {
        case .enabled:
            return "FreeFlow reflects the actual macOS login item state."
        case .disabled:
            return "FreeFlow reflects the actual macOS login item state. Unsigned or development builds may fail to enable this."
        case .requiresApproval:
            return "macOS requires approval for FreeFlow in Login Items before launch at startup becomes active."
        }
    }
}
#endif
